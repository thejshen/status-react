...
(register-handler-fx
  ...
  (fn [{:keys [db]} [_ {:keys [whisper-identity] :as contact}]]
    (when (get-in db [:contacts whisper-identity])
      {:db            (update-in db [:contacts whisper-identity] merge contact)
...
(register-handler-fx
  ...
  (fn [{:keys [db]} [_ {:keys [whisper-identity] :as contact}]]
    (when-not (get-in db [:contacts whisper-identity])
...
(register-handler-fx
  ...
  (fn [{:keys [db]} [_ whisper-identity pred]]
    (let [contact (get-in db [:contacts whisper-identity])]
      (when (and contact (pred contact))
        {:db              (update db :contacts dissoc whisper-identity)
...
(register-handler-fx
  ...
  (fn [{:keys [db]} [_ id]]
    (if (v/is-address? id)
      ...
      {:dispatch (if (get-in db [:contacts id])
