...
(s/def :contact/whisper-identity ::not-empty-string)        ;we can't validate public key, because for dapps whisper-identity is just string
(s/def :contact/name ::not-empty-string)
(s/def :contact/address (s/nilable is-address?))            ;nil for Dapps
(s/def :contact/private-key (s/nilable string?))            ;nil for Dapps
(s/def :contact/public-key (s/nilable string?))             ;nil for Dapps
...
(s/def :contact/debug? boolean?)                            ;true when contact added using status-dev-cli
...
(s/def :contacts/contact-identity (s/nilable ::public-key)) ;on showing this contact profile
(s/def :contacts/contact-list-ui-props (s/nilable (allowed-keys :opt-un [:contact-list-ui/edit?])))
(s/def :contacts/contacts-ui-props (s/nilable (allowed-keys :opt-un [:contacts-ui/edit?])))
(s/def :contacts/contacts-click-handler (s/nilable fn?))    ;used in modal list (for example for wallet)
(s/def :contacts/contacts-click-action (s/nilable #{:send :request})) ;used in modal list (for example for wallet)
