          ...
          (let [path     [(if (= :response (keyword type)) :responses :commands)
                ...
                callback #(let [result (get-in % [:result :returned])
                            ...
                            (when (= :preview data-type)
                              ;; update message in realm with serialized preview (string)
                              (messages/update {:message-id message-id :preview (prn-str result)}))
      ...
      (let [previews (get-in db [:message-data :preview])]
        (when-not (contains? previews message-id)
          (let [{serialized-preview :preview} (messages/get-by-id message-id)]
            ;; if preview is already cached in db, do not request it from jail and write it directly to message-data path
            (if serialized-preview
              (dispatch [:set-in [:message-data :preview message-id] (-> serialized-preview
                                                                        reader/read-string
